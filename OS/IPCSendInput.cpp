...
void IPCSendInput::initBackground()
{
...
IPCSendInput::IPCSendInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void IPCSendInput::update(const float& dt)
{
	...
	if (input.keycounter < 2) {
		...
		PCB::getPCB(System::CPU.getRunningPID())->sendMessage(input.vec.at(0), input.vec.at(1));
		this->states->pop();
	}

}

void IPCSendInput::updateButtons()
{
...
void IPCSendInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void IPCSendInput::updateInput()
{
	this->input.update();
}

void IPCSendInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void IPCSendInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
