...
void PCBSemInput::initBackground()
{
...
PCBSemInput::PCBSemInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void PCBSemInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		PCB::printSemQueue(input.vec.at(0));
		this->states->pop();
	}

}

void PCBSemInput::updateButtons()
{
...
void PCBSemInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void PCBSemInput::updateInput()
{
	this->input.update();
}

void PCBSemInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void PCBSemInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
