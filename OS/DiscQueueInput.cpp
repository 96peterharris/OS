...
void DiscQueueInput::initBackground()
{
...
void DiscQueueInput::initButtons()
{
...
DiscQueueInput::DiscQueueInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void DiscQueueInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		System::FS.printqueue(input.vec.at(0));
		this->states->pop();
	}

}

void DiscQueueInput::updateButtons()
{
...
void DiscQueueInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void DiscQueueInput::updateInput()
{
	this->input.update();
}

void DiscQueueInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void DiscQueueInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
