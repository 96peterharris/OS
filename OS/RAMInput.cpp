...
RAMInput::RAMInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void RAMInput::updateKeybinds(const float& dt)
{
	this->checkForQuit();
}

void RAMInput::update(const float& dt)
{
	...
	if (input.keycounter < 2) {
		...
		try {
			System::RAM.printRam(stoi(input.vec.at(0)), stoi(input.vec.at(1)));
		}
...
void RAMInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void RAMInput::updateInput()
{
	this->input.update();
}

void RAMInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void RAMInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
