...
void PCBInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		PCB::printPCB(input.vec.at(0));
		this->states->pop();
	}

}

void PCBInput::updateButtons()
{
...
void PCBInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void PCBInput::updateInput()
{
	this->input.update();
}

void PCBInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void PCBInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
