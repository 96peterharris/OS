...
void IPCShowMessageInput::initBackground()
{
...
void IPCShowMessageInput::initFonts()
{
...
void IPCShowMessageInput::initButtons()
{
	...
	this->buttons["SHOWMESSAGE"] = new Button(630, 350, 244, 44, &this->font, "Show message");
	//this->buttons["SEND"] = new Button(630, 400, 244, 44, &this->font, "Send");
	//this->buttons["RECEIVE"] = new Button(630, 450, 244, 44, &this->font, "Receive");
}

IPCShowMessageInput::IPCShowMessageInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
IPCShowMessageInput::~IPCShowMessageInput()
{
...
void IPCShowMessageInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		try {
			PCB::showMessages(PCB::getPCB(input.vec.at(0)));
		}
...
void IPCShowMessageInput::updateButtons()
{
...
void IPCShowMessageInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void IPCShowMessageInput::updateInput()
{
	this->input.update();
}

void IPCShowMessageInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void IPCShowMessageInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
