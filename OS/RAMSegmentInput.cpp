...
void RAMSegmentInput::initBackground()
{
...
void RAMSegmentInput::initFonts()
{
...
void RAMSegmentInput::initButtons()
{
...
RAMSegmentInput::RAMSegmentInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
RAMSegmentInput::~RAMSegmentInput()
{
...
void RAMSegmentInput::updateKeybinds(const float& dt)
{
	this->checkForQuit();
}

void RAMSegmentInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		try {
			System::RAM.printSegment(input.vec.at(0), stoi(input.vec.at(1)));
		}
...
void RAMSegmentInput::updateButtons()
{
...
void RAMSegmentInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void RAMSegmentInput::updateInput()
{
	this->input.update();
}

void RAMSegmentInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void RAMSegmentInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
