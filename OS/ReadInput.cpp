...
ReadInput::ReadInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void ReadInput::updateKeybinds(const float& dt)
{
	this->checkForQuit();
}

void ReadInput::update(const float& dt)
{
	...
	if (input.keycounter < 2) {
		...
		try {
			System::FS.readFile(input.vec.at(0), stoi(input.vec.at(1)));
		}
...
void ReadInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void ReadInput::updateInput()
{
	this->input.update();
}

void ReadInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void ReadInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
