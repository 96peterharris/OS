...
void OverwriteFileInput::initBackground()
{
...
void OverwriteFileInput::initFonts()
{
...
void OverwriteFileInput::initButtons()
{
...
OverwriteFileInput::OverwriteFileInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
OverwriteFileInput::~OverwriteFileInput()
{
...
void OverwriteFileInput::update(const float& dt)
{
	...
	if (input.keycounter < 2) {
		...
		System::FS.overwriteFile(input.vec.at(0), input.vec.at(1));
		this->states->pop();
	}
}

void OverwriteFileInput::updateButtons()
{
...
void OverwriteFileInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void OverwriteFileInput::updateInput()
{
	this->input.update();
}

void OverwriteFileInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void OverwriteFileInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
