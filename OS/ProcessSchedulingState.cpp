...
ProcessSchedulingState::ProcessSchedulingState(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void ProcessSchedulingState::updateButtons()
{
	...
	if (this->buttons["PCBQUEUE"]->isPressed() && isMousePressed == false)
	{
		isMousePressed = true;
		System::CPU.displayPCBqueue();
	}

	if (this->buttons["RUNNING"]->isPressed() && isMousePressed == false)
	{
		isMousePressed = true;
