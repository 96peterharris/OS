...
void PCBSegmentsInput::initBackground()
{
...
void PCBSegmentsInput::initFonts()
{
...
void PCBSegmentsInput::initButtons()
{
...
PCBSegmentsInput::PCBSegmentsInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
PCBSegmentsInput::~PCBSegmentsInput()
{
...
void PCBSegmentsInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		System::VM.printPCBsegments(input.vec.at(0));
		this->states->pop();
	}

}

void PCBSegmentsInput::updateButtons()
{
...
void PCBSegmentsInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void PCBSegmentsInput::updateInput()
{
	this->input.update();
}

void PCBSegmentsInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void PCBSegmentsInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
