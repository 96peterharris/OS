...
void RAMMessageInput::initBackground()
{
...
void RAMMessageInput::initFonts()
{
...
void RAMMessageInput::initButtons()
{
...
RAMMessageInput::RAMMessageInput(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
RAMMessageInput::~RAMMessageInput()
{
...
void RAMMessageInput::updateKeybinds(const float& dt)
{
	this->checkForQuit();
}

void RAMMessageInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		try {
			System::RAM.printMessage(stoi(input.vec.at(0)));
		}
...
void RAMMessageInput::updateButtons()
{
...
void RAMMessageInput::updateTimebar()
{
	this->timebar.getActualTime();
}

void RAMMessageInput::updateInput()
{
	this->input.update();
}

void RAMMessageInput::renderButtons(sf::RenderTarget * target)
{
	for (auto &it : this->buttons) {
...
void RAMMessageInput::render(sf::RenderTarget* target)
{
	if (!target) {
		target = this->window;
	}
