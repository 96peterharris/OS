...
DiscState::DiscState(sf::RenderWindow *window, std::stack<GUIState*> *states, sf::Event *event)
...
void DiscState::updateButtons()
{
	...
	{
		isMousePressed = true;
		this->states->push(new DiscBlockInput(this->window, this->states, this->event));
	}
	...
	{
		isMousePressed = true;
		this->states->push(new DiscQueueInput(this->window, this->states, this->event));
	}
